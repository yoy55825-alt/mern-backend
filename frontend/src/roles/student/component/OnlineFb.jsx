...
import './OnlineSubmit.css'; // We'll create this CSS file
...
const OnlineSubmit = () => {
  ...
  // Helper function to render answer review
  const renderAnswerReview = (question, result, index) => {
    if (question.questionType === 'true_false') {
      return (
        <div className="review-answer">
          <i className={`fas ${result?.isCorrect ? 'fa-check-circle' : 'fa-times-circle'}`}></i>
          <span>Your answer: {answers[question._id] === true ? 'True' : answers[question._id] === false ? 'False' : 'Not answered'}</span>
          {!result?.isCorrect && question.correctBoolean !== undefined && (
            <span className="correct-answer">Correct answer: {question.correctBoolean ? 'True' : 'False'}</span>
          )}
      ...
      return (
        <div className="review-answer">
          <i className={`fas ${result?.isCorrect ? 'fa-check-circle' : 'fa-times-circle'}`}></i>
          <span>Your answer: "{answers[question._id] || 'Not answered'}"</span>
          {!result?.isCorrect && question.correctAnswers && question.correctAnswers.length > 0 && (
              ...
              {assignment.questions.map((question, index) => {
                ...
                return (
                  <div key={question._id} className={`review-item ${result?.isCorrect ? 'correct' : 'incorrect'}`}>
                    ...
                    {renderAnswerReview(question, result, index)}
                  </div>
                );
              })}
            </div>
    ...
    <div className="online-submit-container">
            ...
            <div className="question-header">
              <div className="question-number">
                ...
                {question.questionType === 'fill_blank' && (
                  <span className="question-type-badge fill-blank">Fill in the Blank</span>
                )}
      ...
      <div className="submission-footer">
        ...
        <button
          ...
          disabled={submitting || Object.values(answers).filter(a => a !== null && a !== '').length !== assignment.questions?.length}
        >
          {submitting ? (
            <>
              <i className="fas fa-spinner fa-spin"></i>
